...
3. **Proceed with Standard Workflow**
   - Instruct the user to continue with the normal phases (using the actual created directory name):
     - `/spec:requirements {NN}-{feature-slug}` — detail requirements using EARS
     - `/spec:design {NN}-{feature-slug}` — generate design from requirements
     - `/spec:tasks {NN}-{feature-slug}` — create implementation tasks from design
     - `/spec:execute {NN}-{feature-slug}` — implement tasks one at a time (TDD)
...
Next, define the requirements with `/spec:requirements {NN}-{feature-slug}`."
...
💡 **Tip**: After creating the feature, consider using `/clear` before proceeding to `/spec:requirements` for optimal performance.
...
- Next: `/spec:requirements 03-csv-export` → `/spec:design 03-csv-export` → `/spec:tasks 03-csv-export` → `/spec:execute 03-csv-export`
